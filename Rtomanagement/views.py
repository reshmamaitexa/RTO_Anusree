...
class getIndividualTestDates(GenericAPIView):
    serializer_class = Test_dateSerializer 
    def get(self,request,id):
        queryset = Test_date.objects.all().filter(userreg_id=id).values()
...
class HazmateAPIView(GenericAPIView): 
    ...
    def post(self, request):
        ...
        serializer = self.serializer_class(data ={"user_id":user_id,'First_Name': First_Name,'Last_Name':Last_Name,'Date_of_birth':Date_of_birth,'Email_address':Email_address,'Phone_no':Phone_no,'Licence_photo':Licence_photo,'hazmate_status':hazmate_status})
        print(serializer)
...
class getIndividualTestDate(GenericAPIView):
    ...
    def get(self,request):
        queryset = Test_date.objects.all().values()
